...
app.get('/getTodos/page/:nr_skips', async (req,res)=>{
    ...
    Todo.find().skip(Number(skipsElements)).limit(8).sort({_id:-1}).then(recipe => res.json(recipe))
})
...
app.post('/update', async (req,res) =>{
    var id = req.body.id;
    try {
        await Todo.updateOne({_id: objectId(id)},{todo_text: req.body.todo_text},{date_todo: req.body.date_todo})
